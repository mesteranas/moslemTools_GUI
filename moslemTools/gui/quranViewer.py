...
class QuranViewer(qt.QDialog):
    def __init__(self,p,text:str,type:int,category,index=0,enableNextPreviouseButtons=False,typeResult=[],CurrentIndex=0):
        ...
        self.previouse=qt.QPushButton(_("السابق"))
        self.previouse.clicked.connect(self.onPreviouse)
        qt1.QShortcut("alt+left",self).activated.connect(self.onPreviouse)
        self.previouse.setEnabled(enableNextPreviouseButtons)
        buttonsLayout.addWidget(self.previouse)
