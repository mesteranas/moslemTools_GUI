...
class QuranPlayer(qt.QWidget):
    def __init__(self):
        ...
        self.play_all=qt.QPushButton(_("تشغيل كل السور من بداية السورة المركز عليها"))
        self.play_all.setAccessibleDescription("control plus A")
        self.play_all.setCheckable(True)
        self.play_all.setDefault(True)
        self.play_all.setShortcut("ctrl+a")
        ...
        self.play_all.toggled.connect(self.handle_play_all_toggled)
        ...
        bottom_layout.addWidget(self.play_all)
        bottom_layout.addWidget(self.duration)
    ...
    def handle_play_all_toggled(self, checked):
        if checked:            
            if self.listWidget.currentRow() == -1 and self.listWidget.count() > 0:
                self.listWidget.setCurrentRow(0)
            self.play_selected_audio()
    def handle_media_status_changed(self, status):        
        if status == QMediaPlayer.MediaStatus.EndOfMedia and self.play_all.isChecked():
    ...
    def play_next_in_list(self):
        ...
        if current_row<self.listWidget.count() - 1:
            ...
            self.play_all.setChecked(False)
