...
class main(qt.QMainWindow):
    def __init__(self):
        ...
        action_license.setShortcut("ctrl+n")
        ...
        font = qt1.QFont()
        font.setBold(True)        
        self.tray_menu.setAccessibleName(_("تم فتح قائمة moslem tools"))
    ...
    def whats_new_funktion(self):
        try:
            ...
            print(e)
            guiTools.qMessageBox.MessageBox.error(self, _("خطأ"), _("فشلت عملية جلب المعلومات, الرجاء الإتصال بالإنترنت"))
...
if shared.attach() or not shared.create(1):
    guiTools.qMessageBox.MessageBox.error(window,_("تنبيه"),_("البرنامج يعمل بالفعل"))
    sys.exit(0)
